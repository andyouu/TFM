...
def obt_regressors(df,n) -> Tuple[pd.DataFrame, str]:
    ...
    new_df = df[['session', 'outcome', 'side', 'iti_duration','probability_r','prev_opto_bool','opto_bool']]
    new_df = new_df.copy()
    ...
    new_df['opto'] = np.nan
    new_df.loc[new_df['prev_opto_bool'] == 1, 'opto'] = 1
    new_df.loc[new_df['prev_opto_bool'] == 0, 'opto'] = 0 
    ...
    for i in range(1, n+1):
        ...
        new_df[f'prb_r_r_plus{i}'] = new_df[f'r_plus_{i}']*new_df['opto']
        new_df[f'prb_r_r_minus{i}'] = new_df[f'r_minus_{i}']*new_df['opto']
    ...
    for i in range(1, n + 1):
        ...
        #try method
        regr_plus += f'r_plus_{i} + opto * 'f'r_plus_{i} + '
        regr_minus += f'r_minus_{i} + opto * 'f'r_minus_{i} + '
    
    #add the for the block probability regressror
    regressors_string = 'opto + ' + regr_plus + regr_minus[:-3]
    ...
    if expo_yes:
        ...
        regressors_string = 'opto + ' + 'exponent_plus + ' + 'exponent_minus + ' + 'opto * exponent_plus + ' + 'opto * exponent_minus'
...
def plot_GLM(ax, GLM_df, alpha=1):
    ...
    pr_r_plus = GLM_df.loc[GLM_df.index.str.startswith('opto:r_p'), "coefficient"]
    pr_r_minus = GLM_df.loc[GLM_df.index.str.startswith('opto:r_m'), "coefficient"]
...
def glm(df):
    ...
    avaluate = 0 #model valuation 0 if you just want to run the plot
    if not avaluate:
        exponentiate = 0 # tau
        if not exponentiate:
...
if __name__ == '__main__':
    ...
    print(df['subject'])
    df['task'].isin(['S4_5_third_condition'])
